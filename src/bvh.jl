...
@inline function closest(x::SVector{D,T},bvh::ImplicitBVH.BVH,mesh;init_d²=floatmax(T),verbose=false) where {D,T}
    ...
    best = ClosestPoint(Int32(0), T(init_d²), x, x) # initialize best element (promote init_d² so Dual `x` flows through)
    # Depth-First-Search
    ...
    i=Int32(1); while true
        ...
        if j ≤ length_nodes # we are on a node
            ...
            if dist(x, @inbounds bvh.leaves[j-length_nodes]) ≤ best.d²
                ...
                d² = sum(abs2,x-p)
                if d² < best.d²
                    best = ClosestPoint(Int32(j), d², T.(normal(tri)), p) # promote n to T so Dual `x` flows through
                elseif d² ≈ best.d²
                    n = T.(normal(tri))
